...
private val categories = listOf("Enhancement", "New Feature", "UI/UX", "Integration", "Performance")
private val priorities = listOf("Nice to Have", "Important", "Essential")

@OptIn(ExperimentalLayoutApi::class)
@Composable
fun FeatureRequestScreen(settingsDataStore: SettingsDataStore) {
    val scope = rememberCoroutineScope()
    val reportsJson by settingsDataStore.localReports.collectAsState(initial = "[]")

    var title by remember { mutableStateOf("") }
    var category by remember { mutableStateOf("Enhancement") }
    ...
    if (submitted) {
        Column(
            ...
            Text("Feature Request Submitted", style = MaterialTheme.typography.headlineSmall, color = MaterialTheme.colorScheme.onBackground)
            Spacer(Modifier.height(8.dp))
            Text("Thank you for your suggestion!", style = MaterialTheme.typography.bodyMedium, color = MaterialTheme.colorScheme.onSurfaceVariant)
            Spacer(Modifier.height(24.dp))
            Button(
                onClick = { submitted = false; title = ""; description = "" },
    ...
    Column(
        ...
        SectionHeader(title = "Request a Feature")
        ...
        OutlinedTextField(
            ...
            label = { Text("Title") },
            singleLine = true,
            colors = featureTextFieldColors()
        )

        // Category
        CardSurface {
            Column {
                Text("Category", style = MaterialTheme.typography.titleSmall, color = MaterialTheme.colorScheme.onSurface)
                Spacer(Modifier.height(8.dp))
        ...
        CardSurface {
            Column {
                Text("Priority", style = MaterialTheme.typography.titleSmall, color = MaterialTheme.colorScheme.onSurface)
                Spacer(Modifier.height(8.dp))
                FlowRow(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    scope.launch {
                        ...
                        reports.add(
                            LocalReport(
                                ...
                                priority = priority.lowercase().replace(" ", "_"),
...
@Composable
private fun featureTextFieldColors() = OutlinedTextFieldDefaults.colors(
